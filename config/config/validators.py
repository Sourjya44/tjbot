...
class ConfigValidators:
    ...
    @staticmethod
    def validate_gpio_conflicts(config: Dict[str, Any]) -> ValidationResult:
        """
        ...
        # Check speaker + GPIO 18 conflicts
        if config.get('hardware', {}).get('speaker', False):
            if 18 in pins_in_use:
                result.add_warning(
                    f"GPIO 18 is used by {pins_in_use[18]} but speaker is enabled. "
                    f"This may cause audio issues or device malfunction."
                )
...
def validate_config(config: Dict[str, Any]) -> ValidationResult:
    ...
    gpio_result = validators.validate_gpio_conflicts(config)
